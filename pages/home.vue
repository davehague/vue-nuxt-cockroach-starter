      ...
      <div class="px-4 py-6 sm:px-0">
        <div class="border-4 border-dashed border-gray-200 rounded-lg bg-white p-8">
          <div class="flex items-center space-x-4">
            ...
            <div v-else
              class="h-12 w-12 rounded-full bg-blue-500 flex items-center justify-center text-white text-xl font-bold">
              {{ firstInitial }}
            </div>
            <div>
          ...
          <div class="mt-8">
            ...
            <div v-if="dbStatus" class="p-4 bg-gray-50 rounded-md">
              <p>Status: <span class="font-medium"
                  :class="dbStatus.status === 'connected' ? 'text-green-600' : 'text-red-600'">{{ dbStatus.status
                  }}</span></p>
...
import type { DatabaseStatus, DatabaseHealthResponse } from "~/types";
...
onMounted(async () => {
  try {
    const healthCheck = await $fetch<DatabaseHealthResponse>("/api/public/health/database");
    dbStatus.value = {
      status: healthCheck.status === "healthy" ? "connected" : "error",
      time: healthCheck.timestamp,
